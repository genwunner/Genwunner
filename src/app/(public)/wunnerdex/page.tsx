  ...
  return (
    <div className="min-h-screen py-20 px-4">
      <div className="max-w-2xl mx-auto">
        <p className="text-yellow-400 text-xs uppercase tracking-[0.3em] font-bold mb-4 text-center">Fan Registry</p>
        ...
        <div className="flex flex-wrap gap-2 justify-center mb-12">
          {['Early merch access', 'City raid alerts', 'Secret links', 'Fan challenges', 'Unreleased previews'].map(perk => (
            <span key={perk} className="bg-yellow-400/10 border border-yellow-400/30 text-yellow-400 text-xs font-bold uppercase tracking-wide px-3 py-1.5 rounded-full">
              {perk}
            </span>
          ))}
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <label className="flex items-start gap-3 text-sm text-white/60 cursor-pointer p-4 bg-zinc-900 border border-white/10 rounded-xl hover:border-white/20 transition-colors">
            <input type="checkbox" name="want_in_city" value="true" className="accent-yellow-400 w-4 h-4 mt-0.5 flex-shrink-0" />
